...
class _BankAppExampleState extends State<BankAppExample> {
  final String token = 'f8ae90296eaa1544bb15f2453f856b742c97aab5';
  int index = 0;
  Widget? body;
...
class BankPickerWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<ASPSP>>(
      future: apiInterface.getASPSPsForCountry(countryCode: 'se'),
          ...
          itemBuilder: (BuildContext context, int index) => ListTile(
            onTap: () async => launch(
             await apiInterface.fetchRedirectLinkForRequisition(
                requisitionID: (
                await apiInterface.createRequisition(
            ...
            leading: Image.network(banks[index].logoURL,fit: BoxFit.contain,height: 100,width: 100,),
          ),
        );
      },
    );
